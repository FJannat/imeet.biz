<?php
$modal_ids = array('Login','Register','Forgot');
$sizes = array(
	'' => 'Normal',
	'tiny' => 'Tiny',
	'small' => 'Small',
...
?><div class="wrap">
	...
	<h2>
		...
		<a style="color:#fff;border:1px solid #333; background-color:#21759B; text-shadow: none;"class="add-new-h2" href="admin.php?page=<?php echo EASYMODAL_SLUG?>&modal_id=<?php esc_attr_e($modal_id)?>&action=clone&safe_csrf_nonce_easy_modal=<?php echo wp_create_nonce("safe_csrf_nonce_easy_modal")?>">Clone</a>
		<a style="color:#fff;border:1px solid #333; background-color:#900; text-shadow: none;"class="add-new-h2" href="admin.php?page=<?php echo EASYMODAL_SLUG?>&modal_id=<?php esc_attr_e($modal_id)?>&action=delete&safe_csrf_nonce_easy_modal=<?php echo wp_create_nonce("safe_csrf_nonce_easy_modal")?>">Delete</a>
		<?php }?>
	</h2>
			...
			<div id="post-body-content">
				<div class="meta-box-sortables ui-sortable">
					...
					<div class="tabwrapper">
						<form method="post" action="admin.php?page=<?php echo EASYMODAL_SLUG?>&modal_id=<?php esc_attr_e($modal_id)?>">
							<input type="hidden" name="em_settings" value="modal"/>
							<div id="general" class="em-tab">
								<table class="form-table">
									<tbody>
										<tr class="form-field form-required"<?php in_array($modal_id, $modal_ids) ? 'style="display:none;"': ''?>>
											<th scope="row">
												<label for="name"><?php _e('Name', 'easy-modal');?> <span class="description">(required)</span></label>
											</th>
											<td>
												<input type="text" name="name" id="name" <?php echo in_array($modal_id, $modal_ids) ? 'disabled="true" ': ''?>value="<?php esc_attr_e($settings['name'])?>"/>
												<p class="description"><?php _e('Enter a name for your modal.','easy-modal')?></p>
											</td>
										</tr>
			
										<tr class="form-field form-required"<?php echo in_array($modal_id, $modal_ids) ? 'style="display:none;"': ''?>>
											<th scope="row">
											...
											<td>
												<p class="field switch" style="clear:both; overflow:auto; display:block;">
													...
													<input type="checkbox" class="checkbox" id="sitewide" name="sitewide" <?php echo in_array($modal_id, $modal_ids) ? 'disabled="true" ': ''?>value="true" <?php echo $settings['sitewide'] == true ? 'checked="checked"' : '' ?> />
												</p>
										...
										<tr class="form-field form-required"<?php echo in_array($modal_id, $modal_ids) ? 'style="display:none;" ': ''?>>
											...
											<td>
												...
												{
													the_editor($settings['content'], "content");
												}
												else
												{
													wp_editor($settings['content'], "content");
												}
												?>
										...
										<tr class="form-field form-required">
											<th scope="row">
												<label for="content"><?php _e('Theme', 'easy-modal');?></label>
											</th>
											<td>
												<select name="theme" id="theme">
													<?php foreach($themes as $value => $name){?>
													<option value="<?php echo $value?>" <?php echo $value == $settings['theme'] ? 'selected="selected"' : ''?>><?php echo $name?></option>
													<?php }?>
												</select>
												<p class="description"><?php _e('Choose the Theme for this modal.','easy-modal')?></p>
											</td>
										</tr>
									</tbody>
								</table>							
							...
							<div id="options" class="em-tab">
								<table class="form-table">
									<tbody>
										...
										<tr class="form-field form-required">
											<th scope="row">
												<label for="closeDisabled"><?php _e('Disable close button?', 'easy-modal');?></label>
											</th>
											<td>
												<p class="field switch" style="clear:both; overflow:auto; display:block;">
													<label class="cb-enable"><span>On</span></label>
													<label class="cb-disable selected"><span>Off</span></label>
													<input type="checkbox" class="checkbox" id="closeDisabled" name="closeDisabled" value="true" <?php echo $settings['closeDisabled'] == true ? 'checked="checked"' : '' ?> />
												</p>
												<p class="description"><?php _e('Choose whether to show the close button.','easy-modal')?></p>
											</td>
										</tr>
									</tbody>
								</table>							
							...
							<div id="examples" class="em-tab">
								<h4><?php _e('Copy this class to the link/button you want to open this modal.','easy-modal')?><span class="desc">eModal-<?php esc_html_e($settings['id'])?></span></h4>
								<h4>Link Example</h4>
								<a href="#" onclick="return false;" class="eModal-<?php esc_html_e($settings['id'])?>">Open Modal</a>
								<pre>&lt;a href="#" class="eModal-<?php esc_html_e($settings['id'])?>">Open Modal&lt;/a></pre>
								<h4>Button Example</h4>
								<button onclick="return false;" class="eModal-<?php esc_html_e($settings['id'])?>">Open Modal</button>
								<pre>&lt;button class="eModal-<?php esc_html_e($settings['id'])?>">Open Modal&lt;/button></pre>
								<h4>Image Example</h4>
								<img style="cursor:pointer;" src="<?php echo EASYMODAL_URL?>/inc/images/admin/easy-modal-icon.png" onclick="return false;" class="eModal-<?php esc_html_e($settings['id'])?>" />
								<pre>&lt;img src="easy-modal-icon.png" class="eModal-<?php esc_html_e($settings['id'])?>" /></pre>
							</div>
							<div class="submit">
